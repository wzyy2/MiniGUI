...
#define MG_VER                                                \
    MAKE_MG_VER(MINIGUI_MAJOR_VERSION, MINIGUI_MICRO_VERSION, \
...
#undef SetTimer
inline BOOL SetTimer(HWND hWnd, int id, unsigned int speed)
{
    return SetTimerEx(hWnd, id, speed, NULL);
}
#endif

#ifdef TextOut
#undef TextOut
inline int TextOut(HDC hdc, int x, int y, const char* strText)
{
    return TextOutLen(hdc, x, y, strText, -1);
}
#endif
...
#ifdef DrawText
#undef DrawText
inline int DrawText(HDC hdc, const char* pText, int nCount, RECT* pRect,
    UINT nFormat)
{
...
public:
    MGPoint() {}
    MGPoint(int x, int y) { SetPoint(x, y); }
    MGPoint(const POINT& pt) { SetPoint(pt); }
    MGPoint(const MGPoint& mgpt) { SetPoint(mgpt); }
    ~MGPoint() {}
...
public:
    ...
    MGRect(const RECT& rt) { SetRect(rt); }
    MGRect(const RECT* prt) { SetRect(prt); }
    MGRect(const MGRect& mgrt) { SetRect(mgrt); }
    MGRect(const MGRect* pmgrt) { SetRect(pmgrt); }
    MGRect(int w, int h)
    {
        left = top = 0;
        right = w;
    ...
    inline void SetRect(const RECT& rt) { *((RECT*)this) = rt; }
    ...
    inline void InflateRect(int cx, int cy) { ::InflateRect(this, cx, cy); }

    inline void InflateRectToPt(int x, int y) { ::InflateRectToPt(this, x, y); }

    inline BOOL PtInRect(int x, int y) { return ::PtInRect(this, x, y); }
    inline BOOL PtInRect(const POINT& pt) { return ::PtInRect(this, pt.x, pt.y); }
...
public:
    ...
    MGClipRgn(const MGClipRgn& mgrgn) { ::ClipRgnCopy(this, &mgrgn); }
    MGClipRgn(const CLIPRGN& rgn) { ::ClipRgnCopy(this, &rgn); }
    MGClipRgn(const PCLIPRGN pClipRgn) { ::ClipRgnCopy(this, pClipRgn); }
    ...
    inline void Empty() { ::EmptyClipRgn(this); }
    ...
    inline void GetBoundRect(PRECT prect) { ::GetClipRgnBoundRect(this, prect); }

    inline BOOL SetRect(const RECT& rt) { return ::SetClipRgn(this, &rt); }
    inline BOOL SetRect(const MGRect& mgrt) { return ::SetClipRgn(this, &mgrt); }
    inline BOOL SetRect(const PRECT prt) { return ::SetClipRgn(this, prt); }

    inline BOOL SetRgn(const CLIPRGN* prgn) { return ::ClipRgnCopy(this, prgn); }

    inline BOOL IsEmpty() { return ::IsEmptyClipRgn(this); }
    inline BOOL operator!() { return IsEmpty(); }

    inline BOOL AddRect(const RECT& rt) { return ::AddClipRect(this, &rt); }
    inline BOOL AddRect(const MGRect& mgrt) { return ::AddClipRect(this, &mgrt); }
    inline BOOL AddRect(const PRECT prt) { return ::AddClipRect(this, prt); }
    ...
    inline BOOL Subtract(const RECT& rt) { return ::SubtractClipRect(this, &rt); }
    ...
    inline BOOL PtIn(int x, int y) { return ::PtInRegion(this, x, y); }
    inline BOOL PtIn(const POINT pt) { return ::PtInRegion(this, pt.x, pt.y); }
    inline BOOL PtIn(const MGPoint mgpt)
    {
        return ::PtInRegion(this, mgpt.x, mgpt.y);
    }

    inline BOOL RectIn(const RECT& rt) { return ::RectInRegion(this, &rt); }
    inline BOOL RectIn(const MGRect& mgrt) { return ::RectInRegion(this, &mgrt); }
    inline BOOL RectIn(const PRECT prt) { return ::RectInRegion(this, prt); }

    inline void Offset(int x, int y) { ::OffsetRegion(this, x, y); }
    inline void Offset(const POINT pt) { ::OffsetRegion(this, pt.x, pt.y); }
    ...
    inline BOOL Xor(const CLIPRGN* src) { return ::XorRegion(this, this, src); }

    inline void operator+=(const RECT& rt) { AddRect(rt); }
    inline void operator+=(const MGRect& mgrt) { AddRect(mgrt); }
    inline void operator+=(const MGClipRgn& mgrgn) { Union(&mgrgn); }
    inline void operator+=(const CLIPRGN& rgn) { Union(&rgn); }

    inline void operator-=(const RECT& rt) { Subtract(rt); }
    inline void operator-=(const MGRect& mgrt) { Subtract(mgrt); }
    inline void operator-=(const MGClipRgn& mgrgn) { Subtract(&mgrgn); }
    inline void operator-=(const CLIPRGN& rgn) { Subtract(&rgn); }

    inline void operator^=(const MGClipRgn& mgrgn) { Xor(&mgrgn); }
    inline void operator^=(const CLIPRGN& rgn) { Xor(&rgn); }
};

typedef void* (*RUNTIME_OBJ)(void);

#define DECLEAR_RUNTIME(class_name) \
public:                             \
    static void* GetRunTimeClass() { return new class_name; }
