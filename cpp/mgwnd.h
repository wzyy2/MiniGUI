 ...
 */
class MGWnd : public MGVisual {
public:
    MGWnd() { m_hWnd = HWND_INVALID; }
    MGWnd(HWND hWnd)
    {
        m_hWnd = HWND_INVALID;
        Attach(hWnd);
    ...
    inline BOOL IsWindow() { return ::IsWindow(m_hWnd); }

    inline BOOL IsMainWindow() { return ::IsMainWindow(m_hWnd); }
    ...
    inline int GetWindowTextLength() { return SendMessage(MSG_GETTEXTLENGTH); }
    /*
   ...
   */
    ...
    inline BOOL MoveWindow(const RECT* prt, BOOL fPaint = TRUE)
    {
        if (!prt)
            return FALSE;
        return ::MoveWindow(m_hWnd, prt->left, prt->top, RECTWP(prt), RECTHP(prt),
            fPaint);
    }
    inline const char* GetCaption() { return GetWindowCaption(m_hWnd); }
    inline BOOL SetCaption(const char* spCaption)
    {
        return SetWindowCaption(m_hWnd, spCaption);
    }

    inline BOOL SetAlpha(WORD flags, Uint8 alpha)
    {
        return SetMemDCAlpha(m_sndHdc, flags, alpha);
    }

    inline int GetID() { return GetDlgCtrlID(m_hWnd); }

    inline int LoadControls(PCTRLDATA pctrlData, int controls)
    {
        return ::MGLoadControls(m_hWnd, pctrlData, controls);
    }

    inline HWND GetChild(int nItemID) { return GetDlgItem(m_hWnd, nItemID); }

    inline int GetChildInt(int nItemID)
    {
        return (int)GetDlgItemInt(m_hWnd, nItemID, NULL, TRUE);
    }
    inline UINT GetChildUINT(int nItemID)
    {
        return (UINT)GetDlgItemInt(m_hWnd, nItemID, NULL, FALSE);
    }
    inline BOOL SetChildInt(int nItemID, int iValue)
    {
        return SetDlgItemInt(m_hWnd, nItemID, (UINT)iValue, TRUE);
    }
    inline BOOL SetChildInt(int nItemID, UINT uValue)
    {
        return SetDlgItemInt(m_hWnd, nItemID, uValue, FALSE);
    }

    inline int GetChildText(int nItemID, char* txtBuf, int txtMax)
    {
        return GetDlgItemText(m_hWnd, nItemID, txtBuf, txtMax);
    }
    inline char* GetChildText(int nItemID, int* plen)
    {
        return GetDlgItemText2(m_hWnd, nItemID, plen);
    }
    inline BOOL SetChildText(int nItemID, const char* txt)
    {
        return SetDlgItemText(m_hWnd, nItemID, txt);
    }

    inline BOOL IsChildEnable(int nItemID)
    {
        return ::IsWindowEnabled(GetChild(nItemID));
    }
    inline BOOL EnableChild(int nItemID, BOOL fEnable = TRUE)
    {
        return ::EnableWindow(GetChild(nItemID), fEnable);
    }

    inline BOOL IsChildVisible(int nItemID)
    {
        return ::IsWindowVisible(GetChild(nItemID));
    }
    ...
    inline HWND GetNextChild(HWND hchild = (HWND)0)
    {
        return ::GetNextChild(m_hWnd, hchild);
    }
    ...
    inline HWND GetNextTabItem(HWND hWndCtrl, BOOL bPrevious = FALSE)
    {
        return GetNextDlgTabItem(m_hWnd, hWndCtrl, bPrevious);
    }
    ...
    inline void UpdateWindow(BOOL bErase = TRUE)
    {
        return ::UpdateWindow(m_hWnd, bErase);
    }

    inline BOOL EnableWindow(BOOL fEnable = TRUE)
    {
    ...
    inline BOOL GetWindowRect(PRECT prcWnd)
    {
        return ::GetWindowRect(m_hWnd, prcWnd);
    }

    inline int GetWindowBkColor() { return ::GetWindowBkColor(m_hWnd); }

    inline int SetWindowBkColor(int new_bkcolor)
    {
        return ::SetWindowBkColor(m_hWnd, new_bkcolor);
    }

    inline PLOGFONT GetWindowFont() { return ::GetWindowFont(m_hWnd); }

    inline PLOGFONT SetWindowFont(PLOGFONT new_font)
    {
        return ::SetWindowFont(m_hWnd, new_font);
    }

    inline HCURSOR GetWindowCursor() { return ::GetWindowCursor(m_hWnd); }

    inline HCURSOR SetWindowCursor(HCURSOR hNewCursor)
    {
        return ::SetWindowCursor(m_hWnd, hNewCursor);
    }
    ...
    {
        return ::ClientToScreen(m_hWnd, x, y);
    }

    inline void ClientToScreen(PPOINT ppoint)
    {
        return ::ClientToScreen(m_hWnd, &ppoint->x, &ppoint->y);
    }

    inline void ScreenToClient(int* x, int* y)
    {
        return ::ScreenToClient(m_hWnd, x, y);
    }

    inline void ScreenToClient(PPOINT ppoint)
    {
        return ::ScreenToClient(m_hWnd, &ppoint->x, &ppoint->y);
    }

    inline void ScrollWindow(int xoffset, int yoffset, const PRECT prc1 = NULL,
        const PRECT prc2 = NULL)
    {
    ...
    inline BOOL CreateCaret(PBITMAP pBitmap, int width, int height)
    {
        return ::CreateCaret(m_hWnd, pBitmap, width, height);
    }
   ...
   */
    ...
    inline BOOL SetCaretPos(POINT pt)
    {
        return ::SetCaretPos(m_hWnd, pt.x, pt.y);
    }
    ...
    inline BOOL SetCaretBlinkTime(UINT uTime)
    {
        return ::SetCaretBlinkTime(m_hWnd, uTime);
    }

    inline void CheckRadioButton(int idFirstButton, int idLastButton,
        int idCheckButton)
    {
    ...
    inline void CheckDlgButton(int nItemID, int nCheck)
    {
        return ::CheckDlgButton(m_hWnd, nItemID, nCheck);
    }
    inline void SetDlgItemCheck(int nItemID)
    {
        CheckDlgButton(nItemID, BST_CHECKED);
    }
    inline void ClearDlgItemCheck(int nItemID)
    {
        CheckDlgButton(nItemID, BST_UNCHECKED);
    }
    inline void MyCheckRadioButton(int idFirstButton, int idLastButton,
        int idCheckButton)
    {
    ...
    inline int MessageBox(const char* pszCaption = NULL,
        const char* pszText = NULL,
        DWORD dwStyle = MB_OK | MB_ICONINFORMATION)
    {
        return ::MessageBox(m_hWnd, pszText, pszCaption, dwStyle);
    }

    inline int MessageBox(const char* pszCaption, DWORD dwStyle,
        const char* format, ...)
    {
        va_list varg;
    ...
    inline int YesNoBox(const char* pszCaption, const char* format, ...)
    {
        ...
        va_start(varg, format);
        vsprintf(szText, format, varg);
        return MessageBox(pszCaption, szText,
    ...
    // timer
    inline BOOL SetTimer(int id, unsigned int speed)
    {
        return ::SetTimer(m_hWnd, id, speed);
    }
    ...
    inline BOOL SetScrollInfo(int iSBar, const SCROLLINFO* lpsi,
        BOOL fRedraw = TRUE)
    {
    ...
    inline void ScrollWindow(int iOffx, int iOffy, const RECT* rc1 = NULL,
        const RECT* rc2 = NULL)
    {
        ::ScrollWindow(m_hWnd, iOffx, iOffy, rc1, rc2);
    }

    inline void SetCapure() { ::SetCapture(m_hWnd); }
...
//
    ...
    inline BOOL SetWindowElementRenderer(
        const char* rdr_name, const WINDOW_ELEMENT_ATTR* we_attrs = NULL)
    {
    ...
    inline BOOL Create(const char* strCaption, HWND hWndParent, int x, int y,
        int w, int h, DWORD dwStyle, DWORD dwStylEx = 0,
        int Id = -1, DWORD dwAddData = 0)
    {
        if (!::IsWindow(hWndParent))
            return FALSE;

        m_hWnd = CreateWindowEx(GetClass(), strCaption ? strCaption : "",
            dwStyle | WS_CHILD, dwStylEx, Id, x, y, w, h,
            hWndParent, (DWORD)this);
    ...
    inline BOOL Create(HWND hWndParent, int x, int y, int w, int h, DWORD dwStyle,
        DWORD dwStylEx = 0, int Id = -1)
    {
        m_hWnd = CreateWindowEx(GetClass(), "", dwStyle | WS_CHILD, dwStylEx, Id, x,
            y, w, h, hWndParent, (DWORD)this);
    ...
    {
        return MoveWindow(x, y, width, height, bUpdate);
    }

    BOOL GetBound(PRECT prc) { return GetWindowRect(prc); }

    MGVisual* GetParentOwner()
    {
        HWND hwnd = GetParent();
        if (::IsWindow(hwnd))
            return WndFromHandle(hwnd);
        return NULL;
    }

    HWND GetOwnerWindowHandle() { return m_hWnd; }

    BOOL IsMGWnd() { return TRUE; }

protected:
    HWND m_hWnd;
    HDC m_sndHdc;
};
#define DECLARE_CTRL_CLASS(className) \
public:                               \
    const char* GetClass() { return (className); }

////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
/*
 * Class MGContainer extern from MGWnd
 */
#include "mglayout.h"
class MGContainer : public MGWnd {
protected:
    /*
   * Function WndProc
   ...
   */
    virtual BOOL WndProc(int iMsg, WPARAM wParam, LPARAM lParam, int* pret);
    static int _wndProc(HWND hwnd, int iMsg, WPARAM wParam, LPARAM lParam);

    MGLayout* m_layout;

public:
    MGContainer() { m_layout = NULL; }
    MGContainer(HWND hWnd)
        : MGWnd(hWnd)
    {
        m_layout = NULL;
    }
    ...
    void SetLayout(MGLayout* layout)
    {
        ...
        if (m_layout) {
            RECT rc;
            if (GetClientRect(&rc))
                m_layout->SetBound(rc.left, rc.top, RECTW(rc), RECTH(rc));
        }
    }

    MGLayout* GetLayout() { return m_layout; }

    DECLARE_CTRL_CLASS("Container")
 ...
 */
class MGMainWnd : public MGContainer {
public:
    MGMainWnd();
    ~MGMainWnd();
    /*
   ...
   */
    BOOL Create(DWORD dwStyle, const char* spCaption, int x, int y, int rx,
        int by, HWND host = HWND_DESKTOP, HCURSOR hCursor = (HCURSOR)0,
        HMENU hMenu = (HMENU)0, HICON hIcon = (HICON)0,
        int iBackColor = COLOR_lightwhite, DWORD dwStyleEx = WS_EX_NONE,
        int language = 0);
...
#define END_MSG_MAP_PARENT_PROC(ParentCls)                 \
    }                                                      \
    return ParentCls::WndProc(iMsg, wParam, lParam, pret); \
    }
...
#define HOOK_MSG(HookProc)                    \
    ...
    if (HookProc(iMsg, wParam, lParam, pret)) \
        return TRUE;                          \
    switch (iMsg) {
// this marco only used by main window
// BOOL OnMainCreate(PMAINWINCREATE pcreate_info);
#define MAP_MAINCREATE(OnMainCreate)               \
    case MSG_CREATE: {                             \
        if (!OnMainCreate((PMAINWINCREATE)lParam)) \
            RETURN(-1);                            \
        break;                                     \
    }
// this marco only used by control window
// BOOL OnControlCreate(HWND hParent,DWORD dwAddData);
#define MAP_CONTROLCREATE(OnControlCreate)                 \
    case MSG_CREATE: {                                     \
        if (!OnControlCreate((HWND)wParam, (DWORD)lParam)) \
            RETURN(-1);                                    \
        break;                                             \
    }

// BOOL OnNcCreate(MAINWINCREATE *pCreateInfo);
#define MAP_NCCREATE(OnNcCreate)                 \
    case MSG_NCCREATE: {                         \
        if (!OnNcCreate((MAINWINCREATE*)lParam)) \
            RETURN(-1);                          \
        break;                                   \
    }
// BOOL OnInitDialog(HWND focus_hwnd,LPARAM lParam)
#define MAP_INITDIALOG(OnInitDialog)            \
    case MSG_INITDIALOG: {                      \
...
// void OnSetFocus()
#define MAP_SETFOCUS(OnSetFocus) \
    case MSG_SETFOCUS: {         \
        OnSetFocus();            \
        break;                   \
    }
...
// void OnKillFocus()
#define MAP_KILLFOCUS(OnKillFocus) \
    case MSG_KILLFOCUS: {          \
        OnKillFocus();             \
        break;                     \
    }
...
#define MAP_SIZECHANGING(OnSizeChanging)              \
    case MSG_SIZECHANGING: {                          \
        OnSizeChanging((PRECT)wParam, (PRECT)lParam); \
        RETURN(0);                                    \
    }

// BOOL OnSizeChanged(PRECT prcWin,PRECT prcClient); True :Channged, FALSE use
// default;
#define MAP_SIZECHANGED(OnSizeChanged)                         \
...
#define MAP_CSIZECHANGED(OnCSizeChanged)          \
...
#define MAP_MAINWNDCREATE(OnMainWndCreate)           \
    case MSG_CREATE: {                               \
        if (OnMainWndCreate((MAINWINCREATE*)lParam)) \
            RETURN(0);                               \
        RETURN(1);                                   \
    }
// BOOL OnCtrlCreate(HWND hWndParent,DWORD dwAddData);
#define MAP_CTRLCREATE(OnCtrlCreate)                   \
    case MSG_CREATE: {                                 \
        if (OnCtrlCreate((HWND)wParam, (DWORD)lParam)) \
            RETURN(0);                                 \
        return (1);                                    \
    }

// BOOL OnFontChanging(PLOGFONT log_font); TRUE: accept, FALSE refuse
#define MAP_FONTCHANGING(OnFontChanging)      \
    case MSG_FONTCHANGING: {                  \
        if (OnFontChanging((PLOGFONT)lParam)) \
            RETURN(0);                        \
        RETURN(1);                            \
    }
// void OnFontChanged();
#define MAP_FONTCHANGED(OnFontChanged) \
...
#define MAP_ERASEBKGND(OnEraseBkgrnd)                  \
    case MSG_ERASEBKGND: {                             \
        if (OnEraseBkgrnd((HDC)wParam, (PRECT)lParam)) \
            RETURN(0);                                 \
        break;                                         \
    }

#ifdef MSG_ERASEBKGND_N
#define MAP_ERASEBKGNDN(OnEraseBkgrnd)                 \
    case MSG_ERASEBKGND_N: {                           \
        if (OnEraseBkgrnd((HDC)wParam, (PRECT)lParam)) \
            RETURN(0);                                 \
        break;                                         \
    }
...
#define MAP_KEYDOWN(OnKeyDown)              \
    case MSG_KEYDOWN: {                     \
        if (OnKeyDown((int)wParam, lParam)) \
            RETURN(0);                      \
        break;                              \
    }

// BOOL OnKeyUp(int scancode,DWORD key_status); TRUE: user handled
#define MAP_KEYUP(OnKeyUp)                \
    case MSG_KEYUP: {                     \
        if (OnKeyUp((int)wParam, lParam)) \
            RETURN(0);                    \
        break;                            \
    }

// BOOL OnChar(int ch,DWORD key_status); TRUE: user handled
#define MAP_CHAR(OnChar)                 \
    case MSG_CHAR: {                     \
        if (OnChar((int)wParam, lParam)) \
            RETURN(0);                   \
        break;                           \
    }

// BOOL OnSysKeyDown(int scancode,DWORD key_status); TRUE: user handled
#define MAP_SYSKEYDOWN(OnSysKeyDown)           \
    case MSG_SYSKEYDOWN: {                     \
        if (OnSysKeyDown((int)wParam, lParam)) \
            RETURN(0);                         \
        break;                                 \
    }

// BOOL OnSysKeyUp(int scancode,DWORD key_status); TRUE: user handled
#define MAP_SYSKEYUP(OnSysKeyUp)             \
    case MSG_SYSKEYUP: {                     \
        if (OnSysKeyUp((int)wParam, lParam)) \
            RETURN(0);                       \
        break;                               \
    }

// BOOL OnSysChar(int ch,DWORD key_status); TRUE: user handled
#define MAP_SYSCHAR(OnSysChar)              \
    case MSG_SYSCHAR: {                     \
        if (OnSysChar((int)wParam, lParam)) \
            RETURN(0);                      \
        break;                              \
    }

// void OnKeyLongPress();
#define MAP_KEYLONGPRESS(OnKeyLongPress) \
...
#define MAP_KEYALWAYSPRESS(OnKeyAlwaysPress) \
...
#define MAP_LBUTTONDOWN(OnLButtonDown)                                  \
...
#define MAP_LBUTTONUP(OnLButtonUp)                                    \
    case MSG_LBUTTONUP: {                                             \
...
#define MAP_LBUTTONDBLCLK(OnLButtonDblClk)                                \
...
#define MAP_MOUSEMOVE(OnMouseMove)                                    \
    case MSG_MOUSEMOVE: {                                             \
...
#define MAP_RBUTTONDOWN(OnRButtonDown)                                  \
...
#define MAP_RBUTTONUP(OnRButtonUp)                                    \
    case MSG_RBUTTONUP: {                                             \
...
#define MAP_RBUTTONDBLCLK(OnRButtonDblClk)                                \
...
#define MAP_NCLBUTTONDOWN(OnNCLButtonDown)                                \
...
#define MAP_NCLBUTTONUP(OnNCLButtonUp)                                  \
...
#define MAP_NCLBUTTONDBLCLK(OnNCLButtonDblClk)                              \
...
#define MAP_NCMOUSEMOVE(OnNCMouseMove)                                  \
...
#define MAP_NCRBUTTONDOWN(OnNCRButtonDown)                                \
...
#define MAP_NCRBUTTONUP(OnNCRButtonUp)                                \
...
#define MAP_NCRBUTTONDBLCLK(OnNCRButtonDblClk)                            \
...
#define MAP_NCHITTEST(OnNCHitTest)                            \
    case MSG_NCHITTEST: {                                     \
...
#define MAP_MESSAGE(message, OnMessage)         \
    case message: {                             \
        return OnMessage(wParam, lParam, pret); \
    }
// BOOL OnMessageRange(int iMsg,WPARAM wParam,LPARAM lParam,int *pret);
#define MAP_MESSAGE_RANGE(message_begin, message_end, OnMessageRange) \
    case message_begin ... message_end:                               \
...
#define BEGIN_COMMAND_MAP            \
    case MSG_COMMAND: {              \
        ...
        switch (ctrlId) {
// void OnCmd()
#define MAP_COMMAND(id, OnCmd) \
    case id:                   \
        OnCmd();               \
        break;
// void OnCmdEx(int code, HWND hWnd)
#define MAP_COMMANDEX(id, OnCmdEx)   \
    case id: {                       \
        int code = HIWORD(wParam);   \
        OnCmdEx(code, (HWND)lParam); \
    } break;
// void OnCmdRang(int id,int code,HWND hWnd)
#define MAP_COMMAND_RANGE(idbegin, idend, OnCmdRange) \
    case idbegin ... idend:                           \
        OnCmdRange(ctrlId);                           \
        break;
// void OnCmdRangEx(int id,int code,HWND hWnd)
#define MAP_COMMAND_RANGEEX(idbegin, idend, OnCmdRangeEx) \
    case idbegin ... idend: {                             \
        int code = HIWORD(wParam);                        \
...
#define MAP_NOTIFY(id, notify_msg, OnNotify) \
    case id: {                               \
        int code = HIWORD(wParam);           \
        if (code == notify_msg)              \
...
#define BEGIN_NOTIFY_MAP(id)       \
    case id: {                     \
        int code = HIWORD(wParam); \
        switch (code) {
#define MAP_CTRL_NOTIFY(notify_msg, OnNotify) \
    case notify_msg:                          \
        OnNotify((HWND)lParam);               \
        break;
 ...
 */
#define CALL_BASE_CLASS_MAPS(BaseClass) \
    default:                            \
...
public:
    ...
    inline BOOL CreatePopupMenu(const char* strCaption,
        BOOL fStripPopup = FALSE)
    {
        MENUITEMINFO mii;
    ...
    inline BOOL InsertMenuItem(int item, BOOL flag, PMENUITEMINFO pmii)
    {
        return ::InsertMenuItem(m_hMenu, item, flag, pmii) == 0;
    }

    inline BOOL InsertMenuItem(int item, int id, const char* strCaption,
        UINT state = MFS_ENABLED,
        HMENU hSubmenu = (HMENU)NULL)
    {
        MENUITEMINFO mii;
        mii.id = id;
        mii.typedata = (DWORD)strCaption;
    ...
    inline int TrackPopupMenu(int x = 0, int y = 0, HWND hWnd = (HWND)NULL,
        UINT flag = TPM_LEFTALIGN)
    {
    ...
    {
        ::EnableMenuItem(m_hMenu, item,
            MF_BYPOSITION | ((fEnable) ? MFS_ENABLED : MFS_DISABLED));
    }

    inline int GetMenuItemInfo(int item, PMENUITEMINFO pmii,
        BOOL flag = MF_BYCOMMAND)
    {
    ...
    inline int DeleteMenu(int item, BOOL flag = MF_BYCOMMAND)
    {
        return ::DeleteMenu(m_hMenu, item, flag);
    }

    inline int RemoveMenu(int item, BOOL flag = MF_BYCOMMAND)
    {
        return ::RemoveMenu(m_hMenu, item, flag);
    }

    inline int CheckMenuRadioItem(int first, int last, int checkItem,
        UINT flag = MF_BYCOMMAND)
    {
        return ::CheckMenuRadioItem(m_hMenu, first, last, checkItem, flag);
    }

    inline int SetMenuItemBitmaps(int item, UINT flag = MF_BYCOMMAND,
        PBITMAP hBmpUnchecked = NULL,
        PBITMAP hBmpChecked = NULL)
    {
        return ::SetMenuItemBitmaps(m_hMenu, item, flag, hBmpUnchecked,
            hBmpChecked);
    }

    inline int SetMenuItemInfo(int item, PMENUITEMINFO pmii,
        UINT flag = MF_BYCOMMAND)
    {
...
BOOL LoadMenuItem(HMENU hMenu, int iPos, PMUENUITEMTEMPLATE pMenuItemTempl,
    BOOL fStripPopMenu = FALSE);

HMENU LoadPopMenuItem(PPOPMENUTEMPLATE pPopMenuTemplate,
    BOOL fStripPopMenu = FALSE);
...
extern BOOL UpdateHVScroll(HWND hwnd, const RECT* rtdoc, const RECT* rtview,
    int* x_offset, int* y_offset, BOOL bAutoUpdate);

extern int ProcessScrollMessage(HWND hwnd, int iSBar, int nc, int pos, int line,
    BOOL bRedraw);
...
public:
   ...
   */
    BOOL UpdateHVScroll(const RECT* rtdoc, const RECT* rtview = NULL,
        int* x_offset = NULL, int* y_offset = NULL,
        BOOL bAutoUpdate = TRUE)
    {
        return ::UpdateHVScroll(THIS->GetHandle(), rtdoc, rtview, x_offset,
            y_offset, bAutoUpdate);
    }
