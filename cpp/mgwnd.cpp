...
int MGContainer::_wndProc(HWND hWnd, int iMsg, WPARAM wParam, LPARAM lParam)
{
    MGMainWnd* pThis;
...
BOOL MGContainer::WndProc(int iMsg, WPARAM wParam, LPARAM lParam, int* pret)
{
    if (m_layout) {
        if (iMsg == MSG_CSIZECHANGED) {
            RECT rc;
            GetClientRect(&rc);
            m_layout->SetBound(rc.left, rc.top, RECTW(rc), RECTH(rc), TRUE);
            return TRUE;
        } else if (iMsg >= MSG_FIRSTMOUSEMSG && iMsg <= MSG_LASTMOUSEMSG) {
            return m_layout->ProcessMouseMessage(iMsg, LOSWORD(lParam),
                HISWORD(lParam), wParam, pret);
        } else if (iMsg == MSG_PAINT) {
            HDC hdc = BeginPaint();
            Paint(hdc);
            EndPaint(hdc);
        }
...
int MGLoadControls(HWND hWnd, PCTRLDATA pCtrlData, int controls)
{
    ...
    for (i = 0; i < controls; i++) {
        if (pCtrlData[i].class_name) {
            hCtrl = CreateWindowEx(pCtrlData[i].class_name, pCtrlData[i].caption,
                pCtrlData[i].dwStyle | WS_CHILD,
                pCtrlData[i].dwExStyle, pCtrlData[i].id,
                pCtrlData[i].x, pCtrlData[i].y, pCtrlData[i].w,
                pCtrlData[i].h, hWnd, pCtrlData[i].dwAddData);
...
BOOL MGMainWnd::Create(DWORD dwStyle, const char* spCaption, int x, int y,
    int rx, int by, HWND host /*=HWND_DESKTOP*/,
    HCURSOR hCursor /*=(HCURSOR)0*/,
    HMENU hMenu /*=(HMENU)0*/, HICON hIcon /*=(HICON)0*/,
    int iBackColor /*=COLOR_lightwhite*/,
    DWORD dwStyleEx /*=WS_EX_NONE*/, int language)
{
    ...
    CreateInfo.MainWindowProc = MGContainer::_wndProc;
    CreateInfo.lx = x;
    CreateInfo.ty = y;
    CreateInfo.rx = rx;
    CreateInfo.by = by;
    ...
    // CreateInfo.language  = language;
    CreateInfo.dwReserved = 0;

    m_hWnd = CreateMainWindow(&CreateInfo);
    m_sndHdc = GetSecondaryDC(m_hWnd);
...
DWORD MGMainWnd::DoMode()
{
  ...
  hWndOwner =:: GetMainWindowHandle(hWndOwner);*/
    ...
    if (hWndOwner && hWndOwner != HWND_DESKTOP) {
        ...
        while (
            ::PeekPostMessage(&msg, hWndOwner, MSG_KEYDOWN, MSG_KEYUP, PM_REMOVE))
            ;
    }
...
static inline void set_scroll_info(HWND hwnd, int iSbar, SCROLLINFO* psi,
    int doc_size, int view_size,
    BOOL boldShowed)
{
...
BOOL UpdateHVScroll(HWND hwnd, const RECT* rtdoc, const RECT* rtview,
    int* x_offset, int* y_offset, BOOL bAutoUpdate)
{
    ...
    if (dwStyle & WS_HSCROLL) {
        set_scroll_info(hwnd, SB_HORZ, &hsi, h_doc_width, h_view_width,
            dwOldStyle & WS_HSCROLL);
    ...
    if (dwStyle & WS_VSCROLL) {
        set_scroll_info(hwnd, SB_VERT, &vsi, v_doc_height, v_view_height,
            dwOldStyle & WS_VSCROLL);
    } else {
...
int ProcessScrollMessage(HWND hwnd, int iSBar, int nc, int pos, int line,
    BOOL bRedraw)
{
    SCROLLINFO si;
    int nPos;
