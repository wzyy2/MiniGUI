...
private:
    ...
    {
        TMGStaticSubclass<TMGCtrlWnd>* pThis = (TMGStaticSubclass<TMGCtrlWnd>*)GetWindowAdditionalData(hWnd);
        if (pThis) {
            int ret = 0;
            if (iMsg == MSG_DESTROY) {
...
protected:
    ...
    {
        if (m_oldProc)
            return (*m_oldProc)(MGSUBCLASS_HWND, iMsg, wParam, lParam);
        return 0;
    }
...
private:
    static int _newCtrlProc(HWND hWnd, int iMsg, WPARAM wParam, LPARAM lParam)
    {
        TMGDynanicSubclass<TMGCtrlWnd>* pThis = (TMGDynanicSubclass<TMGCtrlWnd>*)GetWindowAdditionalData(hWnd);
        if (pThis) {
            int ret = 0;
            if (pThis->WndProc(iMsg, wParam, lParam, &ret))
                return ret;
...
public:
    ...
    virtual void OnCtrlNotified(MGWnd* sender,
        int id,
        int code,
        DWORD add_data)
        = 0;
...
public:
    ...
    MGStatic() {}
    MGStatic(HWND hParent, int Id)
        : MGCtrlWnd(hParent, Id)
    {
    }
    ~MGStatic() {}
    ...
    {
        return (HICON)SendMessage(STM_SETIMAGE, (WPARAM)hIcon);
    }
...
public:
    ...
    {
        return (int)SendMessage(EM_GETCARETPOS, (WPARAM)pline_pos,
            (LPARAM)pchar_pos);
    }
    ...
    inline int SetSel(int line_pos1,
        int char_pos1,
        int line_pos2,
        int char_pos2)
    {
    ...
    inline int SelectAll() { return SendMessage(EM_SELECTALL); }
    ...
    inline int Copy() { return SendMessage(EM_COPYTOCB); }
    inline int Cut() { return SendMessage(EM_CUTTOCB); }
    inline int Paste() { return SendMessage(EM_INSERTCBTEXT); }
    ...
    inline int GetLineCount() { return SendMessage(EM_GETLINECOUNT); }
    inline int GetLineHeight() { return SendMessage(EM_GETLINEHEIGHT); }
    inline int SetLineHeight(int height)
    {
        return SendMessage(EM_SETLINEHEIGHT, (WPARAM)height);
    }
    inline int LineScroll() { return SendMessage(EM_LINESCROLL); }
    ...
    inline int GetMaxLimit() { return SendMessage(EM_GETMAXLIMIT); }
    inline int SetMaxLimit(int limit)
    {
        return SendMessage(EM_LIMITTEXT, (WPARAM)limit);
    }
    inline void Redo() { SendMessage(EM_REDO); }
    inline void Undo() { SendMessage(EM_UNDO); }
    ...
    inline void RefreshCaret() { SendMessage(EM_REFRESHCARET); }
    ...
    inline int Replace(const char* pFind,
        const char* pReplace,
        BOOL fDown = TRUE,
        BOOL fCaps = FALSE)
    {
    ...
    inline void ReplaceAll(const char* pFind,
        const char* pReplace,
        BOOL fCaps = FALSE)
    {
        SetCaretPos(0, 0);
...
public:
    ...
    inline int GetCheck() { return SendMessage(BM_GETCHECK); }
    inline int SetCheck(int check_state)
    {
        return SendMessage(BM_SETCHECK, (WPARAM)check_state);
    }
    inline int GetState() { return SendMessage(BM_GETSTATE); }
    ...
    inline void Click() { SendMessage(BM_CLICK); }
...
public:
    ...
    inline int SetCurSel(int cursel)
    {
        return SendMessage(LB_SETCURSEL, (WPARAM)cursel);
    }
    ...
    inline int SetTopIndex(int idx)
    {
        return SendMessage(LB_SETTOPINDEX, (WPARAM)idx);
    }
    ...
    inline int GetCaretIndex(int idx) { return SendMessage(LB_GETCARETINDEX); }
...
public:
    MGComboBox(HWND hWnd)
        : MGCtrlWnd(hWnd)
    {
    }
    MGComboBox(HWND hParent, int Id)
        : MGCtrlWnd(hParent, Id)
    {
    }
    MGComboBox() {}
    ...
    inline int SetCurSel(int idx)
    {
        return SendMessage(CB_SETCURSEL, (WPARAM)idx);
    }
    ...
    inline BOOL GetDroppedState()
    {
        return (BOOL)SendMessage(CB_GETDROPPEDSTATE);
    }
    ...
    {
        return SendMessage(CB_GETSPINPACE, (WPARAM)pspin_pace,
            (LPARAM)pspin_fastpace);
    }
...
public:
    MGProgressBar(HWND hWnd)
        : MGCtrlWnd(hWnd)
    {
    }
    MGProgressBar(HWND hParent, int Id)
        : MGCtrlWnd(hParent, Id)
    {
    }
    MGProgressBar() {}
    ...
    inline int SetPos(int nPos) { return SendMessage(PBM_SETPOS, (WPARAM)nPos); }
...
public:
    MGTrackBar(HWND hWnd)
        : MGCtrlWnd(hWnd)
    {
    }
    MGTrackBar(HWND hParent, int Id)
        : MGCtrlWnd(hParent, Id)
    {
    }
    ...
    inline void SetPos(int pos) { SendMessage(TBM_SETPOS, (WPARAM)pos); }
    ...
    inline int SetMin(int min) { return SendMessage(TBM_SETMIN, (WPARAM)min); }

    inline int SetMax(int max) { return SendMessage(TBM_SETMAX, (WPARAM)max); }
};
...
public:
    MGNewToolBar(HWND hWnd)
        : MGCtrlWnd(hWnd)
    {
    }
    MGNewToolBar(HWND hParent, int Id)
        : MGCtrlWnd(hParent, Id)
    {
    }
    MGNewToolBar() {}
    ...
    inline BOOL Create(HWND hWndParent,
        int x,
        int y,
        int w,
        int h,
        ...
        int Id = -1)
    {
        return MGCtrlWnd::Create(NULL, hWndParent, x, y, w, h, dwStyle, dwStylEx,
            Id, (DWORD)pntbi);
    }
    ...
    inline int AddItem(DWORD which,
        DWORD flags,
        int id,
        const char* text,
        const char* tip,
    ...
    inline int SetBitmap(PBITMAP image,
        int h_cell,
        int w_cell,
        int nr_cells,
        int nr_cols = 4)
    ...
    inline PBITMAP SetBitmap(const char* strBmp,
        int h_cell,
        int w_cell,
        int nr_cells,
        int nr_cols = 4)
...
public:
    MGCoolToolBar(HWND hWnd)
        : MGCtrlWnd(hWnd)
    {
    }
    MGCoolToolBar(HWND hParent, int Id)
        : MGCtrlWnd(hParent, Id)
    {
    }
    MGCoolToolBar() {}
...
public:
    MGScrollWnd(HWND hWnd)
        : MGCtrlWnd(hWnd)
    {
    }
    MGScrollWnd(HWND hParent, int Id)
        : MGCtrlWnd(hParent, Id)
    {
    }
    MGScrollWnd() {}
...
public:
    MGScrollView(HWND hWnd)
        : MGScrollWnd(hWnd)
    {
    }
    MGScrollView(HWND hParent, int Id)
        : MGScrollWnd(hParent, Id)
    {
    }
    MGScrollView() {}
    ~MGScrollView() {}
    ...
    inline int SetItemOPS(SVITEM_INITFUNC initItem,
...
public:
    MGTreeView(HWND hWnd)
        : MGCtrlWnd(hWnd)
    {
    }
    MGTreeView(HWND hParent, int Id)
        : MGCtrlWnd(hParent, Id)
    {
    }
    MGTreeView() {}
    ...
    {
        return (GHANDLE)SendMessage(TVM_ADDITEM, (WPARAM)parent,
            (LPARAM)pnewItemInfo);
    }

    inline GHANDLE AddItem(GHANDLE parent,
        const char* text,
        DWORD dwFlags,
        DWORD dwAddData = 0)
    {
    ...
    //#define InsertItem(parent,text,dwFlags,hIconFold,hIconUnfold,dwAddData)
    //AddItem(parent,text,dwFlags,hIconFold,hIconUnfold,dwAddData)
    ...
    inline int SetItemInfo(GHANDLE item,
        const char* text,
        DWORD dwFlags = 0,
        DWORD dwAddData = 0)
    {
...
public:
    MGListView(HWND hWnd)
        : MGCtrlWnd(hWnd)
    {
    }
    MGListView(HWND hParent, int Id)
        : MGCtrlWnd(hParent, Id)
    {
    }
    MGListView() {}
    ...
    inline HLVITEM AddItem(HLVITEM parent,
        int nItem = 0,
        int nItemHeight = 0,
        DWORD itemData = 0,
        DWORD dwFlags = 0)
    ...
    inline int FillSubitem(HLVITEM pi,
        DWORD flags,
        int nItem,
        int subItem,
        const char* pszText,
    ...
    inline void AddColumn(int nCols,
        int width,
        const char* pszHeadText,
        DWORD image,
    ...
    inline void SetSubitemColor(HLVITEM pi,
        int nItem,
        int subItem,
        int nTextColor)
    {
    ...
    inline HLVITEM GetSelectedItem()
    {
        return (HLVITEM)SendMessage(LVM_GETSELECTEDITEM);
    }
...
//			.nTextColor = 0,
    ...
    inline int SetSubitem(HLVITEM pi,
        DWORD flags,
        int nItem,
        int subItem,
        const char* pszText,
        ...
        DWORD image)
    {
        ...
        lvSubitem.pszText = (char*)pszText;
        lvSubitem.nTextMax = pszText ? strlen(pszText) : 0;
        lvSubitem.nTextColor = nTextColor, lvSubitem.image = image;
    ...
    {
        return (BOOL)SendMessage(LVM_SORTITEMS, (WPARAM)sortData,
            (LPARAM)pfnCompare);
    }
    ...
    {
        return (HLVITEM)SendMessage(LVM_GETRELATEDITEM, (WPARAM)related,
            (LPARAM)item);
    }
    inline HLVITEM GetParentItem(HLVITEM item)
    {
        return GetRelatedItem(item, TVIR_PARENT);
    }
    inline HLVITEM GetFirstChildItem(HLVITEM item)
    {
        return GetRelatedItem(item, TVIR_FIRSTCHILD);
    }
    inline HLVITEM GetNextSlibingItem(HLVITEM item)
    {
        return GetRelatedItem(item, TVIR_NEXTSIBLING);
    }
    inline HLVITEM GetPrevSlibingItem(HLVITEM item)
    {
        return GetRelatedItem(item, TVIR_PREVSIBLING);
    }
...
public:
    MGMonthCalendar(HWND hWnd)
        : MGCtrlWnd(hWnd)
    {
    }
    MGMonthCalendar(HWND hParent, int id)
        : MGCtrlWnd(hParent, id)
    {
    }
    MGMonthCalendar() {}
    ...
    inline void SetCurMonth(int month)
    {
        SendMessage(MCM_SETCURMONTH, (WPARAM)month);
    }

    inline int GetCurYear() { return SendMessage(MCM_GETCURYEAR); }
    inline void SetCurYear(int year)
    {
        SendMessage(MCM_SETCURYEAR, (WPARAM)year);
    }
    ...
    inline void GetCurDate(PSYSTEMTIME psysTm)
    {
        SendMessage(MCM_GETCURDATE, 0, (LPARAM)psysTm);
    }
    ...
    inline void SetCurDate(PSYSTEMTIME psysTm)
    {
        SendMessage(MCM_SETCURDATE, 0, (LPARAM)psysTm);
    }

    inline void SetColor(PMCCOLORINFO pmci)
    {
        SendMessage(MCM_SETCOLOR, 0, (LPARAM)pmci);
    }

    inline void SetDayColor(int day, int color)
    {
        SendMessage(MCM_SETDAYCOLOR, (WPARAM)day, (LPARAM)color);
    }
...
public:
    ...
    inline void SetCur(int cur) { SendMessage(SPM_SETCUR, (WPARAM)cur); }

    inline void SetTarget(HWND hWndTarget)
    {
        SendMessage(SPM_SETTARGET, 0, (LPARAM)hWndTarget);
    }
...
#define DECLEAR_SHEETPAGE(class_name) \
public:                               \
    static MGSheetPage* GetSheetPage() { return (MGSheetPage*)(new class_name); }
...
public:
    MGPropSheet(HWND hWnd)
        : MGCtrlWnd(hWnd)
    {
    }
    MGPropSheet(HWND hWndParent, int id)
        : MGCtrlWnd(hWndParent, id)
    {
    }
    MGPropSheet() {}
    ~MGPropSheet() {}
    ...
    inline HWND GetActivePageHandle()
    {
        return (HWND)SendMessage(PSM_GETACTIVEPAGE);
    }
    inline MGUserCtrl* GetActivePage()
    {
        return (MGUserCtrl*)MGWnd::WndFromHandle(GetActivePageHandle());
    }
    inline int GetActiveIndex() { return (int)SendMessage(PSM_GETACTIVEINDEX); }
    inline BOOL SetActiveIndex(int nPage)
    {
        return SendMessage(PSM_SETACTIVEINDEX, (WPARAM)nPage) == PS_OKAY;
    }
    inline HWND GetPageHandle(int index)
    {
        return (HWND)SendMessage(PSM_GETPAGE, (WPARAM)index);
    }
    inline MGSheetPage* GetPage(int index)
    {
        return (MGSheetPage*)MGWnd::WndFromHandle(GetPageHandle(index));
    }
    inline BOOL GetPageIndex(HWND hWndPage)
    {
        return (SendMessage(PSM_GETPAGEINDEX, (WPARAM)hWndPage) == PS_OKAY);
    }
    inline int GetPageCount() { return (int)SendMessage(PSM_GETPAGECOUNT); }
    inline int GetPageTitleLength(int index)
    {
        return (int)SendMessage(PSM_GETTITLELENGTH, (WPARAM)index);
    }
    inline BOOL GetPageTitle(int index, char* buffer)
    {
        return (SendMessage(PSM_GETTITLE, (WPARAM)index, (LPARAM)buffer) == PS_OKAY);
    }
    ...
    inline BOOL SetPageTitle(int index, const char* strTitle)
    {
        return (SendMessage(PSM_SETTITLE, (WPARAM)index, (LPARAM)strTitle) == PS_OKAY);
    }
    ...
    inline int AddPage(DLGTEMPLATE* pdlg_tmp,
        PSHEETPAGE_FACTORY SHEETPAGE_FACTORY)
    {
        if (SHEETPAGE_FACTORY == NULL)
            return -1;
    ...
    inline BOOL RemovePageHandle(int index)
    {
        return (SendMessage(PSM_REMOVEPAGE, (WPARAM)index) == PS_OKAY);
    }
